...
r0 <- defaultgrid(lonlat, prj = "EPSG:4326")
...
akifun <- function(xy, value, grid = NULL, ...) {
  if (is.null(grid)) grid <- defaultgrid(xy)
  ## interp() wants both output axes increasing, and yFromRow() runs top-down.
  ## Hand it a descending yo and it returns a grid of all NA, without complaint.
  aklin <- interp(xy[,1], xy[,2], value,
                  xFromCol(grid), rev(yFromRow(grid)), ...)
  raster(list(x = aklin$x, y = aklin$y, z = aklin$z))
}
